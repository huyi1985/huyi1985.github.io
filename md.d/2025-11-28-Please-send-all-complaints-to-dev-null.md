...
![](/assets/1-Ew_UUSAdLv53qoD-OCbggw.png)
...
![](/assets/1-PrCOyc5scCH_GPteaEoy6A.webp)
...
![Giacomo Tesio's user avatar](/assets/3daaea0ebce68c00202f395eb796724c.png)
...
/dev/null 的 Wikipedia 条目，以及 Null-device 的历史说明，只说了 /dev/null 是 Unix 与类 Unix 系统中一个用来丢弃数据的“黑洞设备 (null device / bit bucket)”。它确实是很多程序员用来幽默表示“把不想要的输出／反馈丢掉” 的隐喻。 
Wikipedia
+2
Wikipedia
+2

但我没能在有关 BSD / Berkeley Unix 历史 (如 BSD 中文社区、BSD 发展史、CSRG 历程) 文档中，看到有提及“用户投诉 / 反馈”曾被系统性地“重定向到 /dev/null”的做法。相关 BSD 历史资料主要聚焦于代码许可、移植、功能加入 (虚存、TCP/IP、文件系统等) 和法律纠纷 (Unix System Laboratories vs Berkeley Software Design, Inc.) 。 
book.bsdcn.org
+2
eol.shzu.edu.cn
+2

我在 “/dev/null” 的文化/行话解释里看到，人们确实会用 “send complaints to /dev/null” 或 “my mail got archived to /dev/null” 这样的表达。 
Wikipedia
+2
Stack Overflow
+2

但这些都是比喻／幽默语境 (“请把投诉发到 /dev/null” = “别发，我不想理你／我不想看”)，并不意味着现实中某个机构/团队真的把用户反馈写入 /dev/null，即系统抛弃。

🧩 为什么很可能只是传说／隐喻

/dev/null 作为“丢弃设备 (null device)” 的作用在 Unix 起源阶段就被定义 — 当初只是操作系统层面的一个 device，用于丢弃输出流。它的存在和用途是技术层面的。 
Wikipedia
+2
Wikipedia
+2

“把投诉丢到 /dev/null”更像是程序员社区里的幽默比喻 (analogous metaphor) — 就像“把意见放到黑洞里 / 垃圾桶里”，用以表达 “这个反馈不被理会 / 我不想看 / 我忽略了”。很多后来文中以这种说法描述 “不被重视 / 被忽略 / 被弃置”。这不是说他们真的重定向系统日志，而是用 /dev/null 作为象征。甚至有现代文章 (“Please send all complaints to /dev/null”) 直接说明这种说法是 jokes / memes。 
Medium
+2
Unix & Linux Stack Exchange
+2

历史上 BSD/UNIX 的维护、开发过程虽然曾因资源、精力有限而疏于用户支持，但现存公开资料更强调 code contributions、协议、法律、发行等 — 没有看到可信文档说明 “系统化地把用户反馈写 /dev/null”。

✅ 我的判断

不能确认有可靠出处表明 “伯克利 Unix 团队把用户投诉／反馈导入到 /dev/null” 是真实事件。

这个说法更像是 程序员行话／社区笑话 (jargon or meme)，用 /dev/null 的语义 (“丢弃、不保留、不理睬”) 来形象表达 “不处理用户反馈 / 忽视用户抱怨”。

换句话说，它不是历史事实，而是一种比喻 — 所以不能当作 “正规事件 / 历史记录” 来引用。
